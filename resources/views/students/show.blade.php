    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between"><span class="text-gray-500">LIN (EMIS)</span><span class="text-gray-900">{{ $student->lin ?? '-' }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">UNEB Index</span><span class="text-gray-900">{{ $student->uneb_index_number ?? '-' }}</span></div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            {{-- Previous School --}}
            {{-- CHANGED: show previous school details + attachment --}}
            @if($student->previous_school || $student->previous_school_grade || $student->previous_school_attachment)
            <div class="bg-white rounded-xl shadow-sm border p-6">
                <h3 class="font-semibold text-gray-800 mb-3">Previous School</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div><span class="text-gray-500 block">School Name</span><span class="text-gray-900">{{ $student->previous_school ?? '-' }}</span></div>
                    <div><span class="text-gray-500 block">Grade / Class</span><span class="text-gray-900">{{ $student->previous_school_grade ?? '-' }}</span></div>
                    <div>
                        <span class="text-gray-500 block">Attachment</span>
                        @if($student->previous_school_attachment)
                        <a href="{{ Storage::url($student->previous_school_attachment) }}" target="_blank" class="font-medium hover:underline" style="color: var(--primary-color);">View document</a>
                        @else
