<x-app-layout>
    ...
    {{-- CHANGED (rebuilt Jul 18): a code formatter reformatted this file as JavaScript and destroyed it.
         Rebuilt with ALL script data prepared in ExamController::create() — no PHP expressions live in
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="academic_year_id" id="academic_year_id" required class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                        ...
                        <option value="{{ $year->id }}" {{ old('academic_year_id', $currentYear?->id) == $year->id ? 'selected' : '' }}>{{ $year->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="assessment_format" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                        <option value="" {{ old('assessment_format', '') === '' ? 'selected' : '' }}>Auto (based on class)</option>
                        <option value="primary" {{ old('assessment_format') === 'primary' ? 'selected' : '' }}>Primary</option>
                        <option value="o-level" {{ old('assessment_format') === 'o-level' ? 'selected' : '' }}>O-Level</option>
                        <option value="a-level" {{ old('assessment_format') === 'a-level' ? 'selected' : '' }}>A-Level</option>
                    </select>
                </div>
                <div>
    ...
    {{-- All constants below come from ExamController::create(); keep this block free of PHP expressions. --}}
    <script>
        ...
        // Restore rows lost to a validation failure, otherwise start with one empty row.
        if (savedComponents.length) {
            savedComponents.forEach(c => addComponent(c.exam_id, c.weight));
        } else {
            addComponent();
        }
    </script>
