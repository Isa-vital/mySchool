<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Fee Structure</h2>
                ...
                <div>
                    ...
                    <select name="fee_type_id" required class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                        ...
                        @foreach($feeTypes as $t)
                            <option value="{{ $t->id }}" {{ old('fee_type_id') == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Amount <span class="text-red-500">*</span></label>
                    <input type="number" name="amount" value="{{ old('amount') }}" required step="0.01" min="0" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
