<x-app-layout>
    ...
    <form method="POST" action="{{ route('timetable.store') }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ old('start_time') }}" required class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">End Time <span class="text-red-500">*</span></label>
                    <input type="time" name="end_time" value="{{ old('end_time') }}" required class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="staff_id" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('staff_id') == $teacher->id ? 'selected' : '' }}>{{ $teacher->first_name }} {{ $teacher->last_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Room</label>
                    <input type="text" name="room" value="{{ old('room') }}" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="px-6 py-2 text-sm font-medium text-white rounded-lg shadow-sm" style="background: var(--primary-color);">Add Slot</button>
        </div>
    </form>
</x-app-layout>
