        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
            <form method="POST" action="{{ route('roles.store') }}">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full md:w-1/2 rounded-lg border-gray-300 text-sm" required>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    @foreach($perms as $perm)
                                        <label class="flex items-center space-x-2 text-sm">
                                            <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" class="perm-checkbox rounded border-gray-300 text-indigo-600" {{ in_array($perm->id, old('permissions', [])) ? 'checked' : '' }}>
                                            <span>{{ $perm->name }}</span>
                                        </label>
                                    @endforeach
                                </div>
                ...
                <div class="mt-6 flex justify-end">
                    <button type="submit" class="px-6 py-2 text-sm font-medium text-white rounded-lg" style="background-color: var(--primary-color);">Create Role</button>
                </div>
            </form>
        </div>
    </div>
