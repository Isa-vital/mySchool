<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Guardian</h2>
    ...
    <form method="POST" action="{{ route('guardians.store') }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="relationship" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                        ...
                        @foreach(['father','mother','uncle','aunt','guardian','other'] as $r)
                            <option value="{{ $r }}" {{ old('relationship') === $r ? 'selected' : '' }}>{{ ucfirst($r) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="occupation" value="{{ old('occupation') }}" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                </div>
        ...
        <div class="flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="px-6 py-2 text-sm font-medium text-white rounded-lg shadow-sm" style="background: var(--primary-color);">Save Guardian</button>
        </div>
    </form>
</x-app-layout>
